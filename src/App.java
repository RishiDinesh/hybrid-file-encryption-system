...
class AESRunnable implements Runnable {
    ...
    public void run() {
        if (mode == "encrypt")
            ...
            result = aes.decrypt(data, initvector);
    }
...
class BlowfishRunnable implements Runnable {
    ...
    public void run() {
        if (mode == "encrypt")
            result = blowfish.encrypt(data);
        else if (mode == "decrypt")
            result = blowfish.decrypt(data, randbin);
    }
...
class ProposedSystem {
    ...
    static void encrypt(String inputFile, String[] keyArray, String outputFile) throws Exception {
        ...
        ArrayList<String> encrypted_strings = TranspositionCipher.encrypt(String.join("", result), keyArray);
        String encrypted_data = String.join("", encrypted_strings).toUpperCase();
        FileWriter out = new FileWriter(outputFile);
        out.write(encrypted_data);
        out.close();
    }
    ...
    static void decrypt(String inputFile, String[] keyArray, String outputFile) throws Exception {
        ...
        String[] data = input.split("(?<=\\G.{" + (int) (Math.ceil(input.length() / 64d) * 32) + "})");
...
class Metrics {
    ...
    static void log(String filename, long startTime, long endTime, String[] args) {
        ...
        Date date = new Date();
        String log = System.lineSeparator() + formatter.format(date) + " : " + String.join(" ", args) + " -tt "
                + timeTaken + " -tp " + throughput;
...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        // Metrics.computeAvalancheEffect("data\\100Kb\\file1.enc",
        // "data\\100Kb\\file2.enc");
        ...
        if (args[2].equals("base")) {
            ...
            if (args[3].equals("AES")) {
                ...
                if (args[0].equals("encrypt"))
                    BaseSystem.encrypt(plainfile, keyArray, encFile, "AES");

                else if (args[0].equals("decrypt"))
                    BaseSystem.decrypt(encFile, keyArray, decFile, "AES");

            } else {

                if (args[0].equals("encrypt"))
                    BaseSystem.encrypt(plainfile, keyArray, encFile, "Blowfish");

                else if (args[0].equals("decrypt"))
                    BaseSystem.decrypt(encFile, keyArray, decFile, "Blowfish");
            ...
            if (args[0].equals("encrypt"))
                ProposedSystem.encrypt(plainfile, keyArray, encFile);

            else if (args[0].equals("decrypt"))
                ProposedSystem.decrypt(encFile, keyArray, decFile);
        ...
        if (args[0].equals("encrypt"))
            Metrics.log(plainfile, startTime, endTime, args);
        else
            Metrics.log(encFile, startTime, endTime, args);
    }
}
